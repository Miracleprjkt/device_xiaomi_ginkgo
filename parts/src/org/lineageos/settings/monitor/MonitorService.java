...
public class MonitorService extends Service {
    ...
    private void monitorForegroundApp() {
        ...
        boolean masterEnabled = prefs.getBoolean("monitor_enable", false);
        if (masterEnabled) {
            Monitor.getInstance(this).applyPreferences();
            Monitor.getInstance(this).show();
            return;
        }
        
        boolean autoEnabled = prefs.getBoolean("monitor_auto_enable", false);
        if (!autoEnabled) {
            Monitor.getInstance(this).hide();
            return;
        }
        
        String foreground = ForegroundAppDetector.getForegroundPackageName(this);
        Set<String> autoApps = prefs.getStringSet(MonitorAppSelectorFragment.PREF_AUTO_APPS, new HashSet<>());
